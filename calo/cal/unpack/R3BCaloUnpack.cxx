...
{
   LOG(DEBUG2) << "R3BCaloUnpack::ctor()";
   LOG(INFO) << type <<", "<<subType  <<", "<<procId<<", "<<subCrate <<", "<<control ;
}
...
{
  LOG(DEBUG2) << "R3BCaloUnpack: Delete instance";
  delete fRawData;
}
...
void R3BCaloUnpack::SetParContainers() {
  ...
  if ( fCaloUnpackPar ) {
    LOG(INFO) << "R3BCaloUnpack::SetParContainers() ";
    LOG(INFO) << "Container R3BCaloUnpackPar loaded ";
  }
}
...
Bool_t R3BCaloUnpack::DoUnpack(Int_t *data, Int_t size) {
  ...
  LOG(DEBUG2) << "R3BCaloUnpack::DoUnpack()";
...
//      << "       data_size " << data_size << FairLogger::endl;
    ...
    if (num_pileup)        error |= 1<<3;
     ...
     LOG(DEBUG2) << "R3BCaloUnpack::DoUnpack(): New Hit: Crystal Id: " << crystal_id << ", fNHits: " << fNHits;
     fNHits++;
