...
EnsarModule::EnsarModule( const Char_t* Name, const Char_t* title, Bool_t Active)
...
void EnsarModule::SetRotAnglesEuler(Double_t phi, Double_t theta, Double_t psi)
{
  ...
  if ( fLocalTrans == kFALSE ){
    ...
    LOG(INFO) << "EnsarModule::SetRotAngles() is ignored ! ";
    LOG(INFO) << "EnsarModule::SetRotAngles Local Transf. already activated ... ";
    fLabTrans = kFALSE;
  }
}
...
void EnsarModule::SetTranslation(Double_t tx, Double_t ty, Double_t tz)
{
  fX = tx;
  fY = ty;
  fZ = tz;
...
void EnsarModule::SetRotAnglesXYZ(Double_t xx, Double_t yy, Double_t zz)
{
  ...
  if ( fLabTrans == kFALSE ){
    ...
    LOG(INFO) << "EnsarModule::SetRotAnglesXYZ() is ignored ! ";
    LOG(INFO) << "EnsarModule::SetRotAngles Lab Transf. already activated ... ";
    fLocalTrans = kFALSE;
  }
}
...
TGeoCombiTrans* EnsarModule::GetGlobalPosition(TGeoCombiTrans *fRef)
{
  if (fLocalTrans == kTRUE ){
    ...
    if (fRefRot) {
      ...
      LOG(ERROR) << "EnsarModule::GetGlobalPosition() \
